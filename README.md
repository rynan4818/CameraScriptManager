...
* **更新チェック** — 起動時に 1 日 1 回以下で最新版を確認し、新しい `CameraScriptManager` があればリリース URL を案内
...
| ボタン | 説明 |
|---|---|
| **スキャン** | CustomLevels / CustomWIPLevelsフォルダをスキャンし、カメラスクリプトの一覧を更新します |
...
`SongScripts` フォルダ内の JSON / ZIP を一覧管理し、metadata の編集保存、未取得譜面の取得、プレイリスト作成を行うタブです。

#### ツールバーボタン

| ボタン | 説明 |
|---|---|
| **スキャン** | SongScripts フォルダをスキャンして一覧を更新します |
| **BeatSaverからmetadata取得** | 選択行の ID を使って BeatSaver から metadata を取得します |
| **保存チェックを保存** | 保存チェックが ON の行を元ファイルへ保存します |
| **プレイリストを作成** | 選択中の SongScripts 行から Beat Saber プレイリスト(.bplist)を作成します |
| **譜面metadataを表示** | 譜面 metadata 列の表示/非表示を切り替えます |
...
| **現在のバージョン / リリースURL** | CameraScriptManager の現在バージョンと、最新版リリースページの URL を表示します |
| **自動アップデートチェック** | ONの場合、起動時に `https://rynan4818.github.io/release_info.json` を 1 日 1 回以下で確認し、新しいバージョンがあればリリース URL を表示します |
