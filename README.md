...
* **Manager機能** — CustomLevels / CustomWIPLevelsフォルダ内の既存カメラスクリプトを一覧表示し、メタデータの編集・追加、ZIP配布用アーカイブの作成、BeatSaberプレイリスト(.bplist)の生成が可能
...
* **リネーム機能** — コピー時にファイル名を自動リネーム（SongScript / Author_ID_SongName / カスタム書式に対応）
* **上書きバックアップ** — 既存ファイルの上書き時に日時付き.bakファイルとして自動退避
...
| **メタ情報追加** | 「選択」チェックが入った項目のSongScript.jsonに、編集したメタデータを書き込みます。変更前のファイルはバックアップされます |
| **ZIP出力** | 選択した項目をZIPアーカイブとしてエクスポートします。配布用パッケージの作成に使用します |
...
| **リネーム** | コピー時のファイルリネーム方式をドロップダウンで選択します。`無し`（元ファイル名のまま）/ `SongScript`（SongScript.jsonにリネーム）/ `AuthorIdSongName`（Author_ID_SongName_SongScript.json形式）/ `カスタム`（Settings で設定した書式）から選択可能 |
...
| **選択したIDをコピー** | 選択行のBeatSaver IDをクリップボードにコピーします |
| **Rename** > **無し / SongScript / Author_ID_SongName** | 選択行のリネーム方式を変更します |
...
| **元データ検索(アーカイブ/JSON) 1〜3** | Managerタブの「元データ照合」で使用する検索対象フォルダを最大3つ指定できます。カメラスクリプトの元アーカイブやJSONファイルが保存されているフォルダを設定します |

#### 命名規則設定

| 設定項目 | 説明 |
|---|---|
| **Manager ZIP出力名** | ManagerタブのZIPエクスポート時のフォルダ名形式を指定します。`デフォルト`（{MapId}\_{SongName}\_{LevelAuthorName}）またはカスタム書式から選択します |
| **Copier リネーム名** | Copierタブでリネーム方式「カスタム」を選択した場合のファイル名の書式を指定します。デフォルトは`{CameraScriptAuthorName}_{MapId}_{SongName}_SongScript`です |
...
| 設定項目 | 説明 |
|---|---|
| **Copier上書き時にバックアップ(.bak)を作成する** | ONの場合、コピー先に既にファイルが存在する場合に、古いファイルに日時（yyyyMMdd_HHmmss.bak）を付与して退避します |
...
Managerタブで「選択」チェックが入った項目から、BeatSaberのプレイリストファイル（.bplist）を生成できます。作成ダイアログでは以下の情報を入力します：
